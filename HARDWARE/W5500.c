...
/***************----- 端口数据缓冲区 -----***************/
unsigned char Rx_Buffer[S_RX_SIZE];	//端口接收数据缓冲区 
unsigned char Tx_Buffer[S_TX_SIZE];	//端口发送数据缓冲区 

unsigned char W5500_Interrupt;	//W5500中断标志(0:无中断,1:有中断)

unsigned char W5500_Unconnect = 0;  //W5500复位后是否连接上标志(1:无连接,0:有连接)
...
* 描述    : 中断线7中断服务函数(W5500 INT引脚中断服务函数)
* 输入    : 无
* 输出    : 无
* 返回值  : 无
* 说明    : 无
...
void Write_SOCK_Data_Buffer(SOCKET s, unsigned char *dat_ptr, unsigned short size)
{
	...
	if(S0_Mode == UDP_MODE && (Read_W5500_SOCK_1Byte(s,Sn_MR)&0x0f) != SOCK_UDP)//如果Socket打开失败
	{		
...
IntDispose:
	...
	{
		 //自己添加代码
        cDebug("Error:W5500 IP Confilict!\r\n");
	}

	if((i & UNREACH) == UNREACH)//UDP模式下地址无法到达异常处理
	{
		//自己添加代码
        cDebug("Error:W5500 UDP Unreach!\r\n");
